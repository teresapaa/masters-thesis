// nvcc --extended-lambda -G -arch=sm_86 -std=c++17 -Xcompiler "/std:c++17" GPU-solution-v2.cu -o GPU-solution-v2.exe
...
__global__ void value_function_iteration_kernel(
    ...
    //finding the indices
    int ij_base = blockIdx.x * blockDim.x + threadIdx.x;
    int i = ij_base / n_k;
    int j = ij_base % n_k;
    if (i >= n_k || j >= n_k) return;


    cuda::atomic_ref<float> V_new_atomic(V_new[i]);
    cuda::atomic_ref<int> policy_atomic(policy[i]);

    //calculate the value for the i,j pair
    ...
    //use shared memory to store max value and best j
    if (V_new_atomic.fetch_max(value) < value) {
        policy_atomic.store(j);
    }
    ...
    void run_compute() {
        ...
        while (diff > epsilon && iteration < max_iter && ++iteration) {
            ...
            int numBlocks = (n_k * n_k + threadsPerBlock - 1) / threadsPerBlock;

			// Reset V_new to NEG_INF before each iteration
            thrust::fill(d_V_new.begin(), d_V_new.end(), NEG_INF);
